...
class RBP(Metric):
    """
    This class implements Ranked biased Precision (RBP) with several parameters. We implemented RBP as in:
    ...
    The use of different values of p reflects different ways in which ranked lists
    can be used. Values close to 1.0 are indicative of highly persistent users, who
    scrutinize many answers before ceasing their search. For example, at p = 0.95,
    there is a roughly 60% likelihood that a user will enter a second page of 10
    results, and a 35% chance that they will go to a third page. Such users obtain
    a relatively low per-document utility from a search unless a high number of
    relevant documents are encountered, scattered through a long prefix of the
    ranking.
    ...
    p: float
        This parameter which simulates user type, and consequently the probability that a viewer actually inspects
    ...
    def __init__(self, name='RBP', cutoff=None, threshold=1, p=0.5):
        """
        This is the constructor of RBP, an object of type Metric, with the name RBP.
    ...
    def eval_per_query(self, y, y_pred):
        """
        This method helps compute the RBP score per query. It is called by the eval function which averages and
